...
export function ParticipantRegistrationManager({ onClose }: ParticipantRegistrationManagerProps) {
  ...
  // Loading states
  const isLoadingRegistrations = allRegistrations === undefined;
  const isLoadingStats = registrationStats === undefined;
  ...
  if (isLoadingRegistrations || isLoadingStats) {
    return (
      <div className="fixed inset-0 bg-charcoal/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-dark-blue/95 backdrop-blur-md border border-medium-blue/30 rounded-2xl p-8 text-center">
          ...
          <h3 className="text-xl font-bold text-silver mb-2">Loading Registrations...</h3>
          <p className="text-silver/70">Please wait while we fetch participant registrations</p>
        </div>
      </div>
    );
  }
