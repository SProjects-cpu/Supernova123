...
export function NewsListManager({ authorEmail, authorName, isAdmin = false }: NewsListManagerProps) {
  ...
  // Loading states
  const isLoadingNews = allNews === undefined;
  const isLoadingStats = newsStats === undefined;
  ...
  if (isLoadingNews || isLoadingStats) {
    return (
      <div className="space-y-6">
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-8 text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-accent-blue mx-auto mb-4"></div>
          <h3 className="text-xl font-bold text-silver mb-2">Loading News Data...</h3>
          <p className="text-silver/70">Please wait while we fetch the latest news updates</p>
        </div>
      </div>
    );
  }
