...
export function NewsUpdatesSection() {
  ...
  // Loading states
  const isLoadingNews = publishedNews === undefined;
  const isLoadingFeatured = featuredNews === undefined;
  ...
  if (isLoadingNews || isLoadingFeatured) {
    return (
      <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8 bg-white/5 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto text-center">
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-starlight-white mb-8 sm:mb-12">
            📰 News & Updates
          </h2>
          <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-8">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-accent-blue mx-auto mb-4"></div>
            <h3 className="text-xl font-bold text-silver mb-2">Loading News...</h3>
            <p className="text-silver/70">Please wait while we fetch the latest updates</p>
          </div>
        </div>
      </section>
    );
