...
interface EventSpecificRegistrationFormProps {
  ...
  onClose: () => void;
  onBack: () => void;
  event?: any; // Preloaded event data
}
...
export function EventSpecificRegistrationForm({ eventId, onClose, onBack, event: preloadedEvent }: EventSpecificRegistrationFormProps) {
  // Use preloaded event if available, otherwise fetch it
  const fetchedEvent = useQuery(api.events.getById, { id: eventId });
  const event = preloadedEvent || fetchedEvent;
  ...
  useEffect(() => {
    if (event) {
      // Show loading for a brief moment even with preloaded data for better UX
  ...
  const teamRules = useMemo(() => {
    ...
    if (eventTitle.includes("battleclipse") || eventTitle.includes("esports")) {
      // Battleclipse: compulsory team of exactly 4 for BGMI, 5 for Valorant
      requireTeam = true;
  ...
  useEffect(() => {
    ...
    // For esports, determine team size based on selected game
    let enforcedSize = minSize ?? formData.teamSize;
    if (eventTitle.includes("battleclipse") || eventTitle.includes("esports")) {
      if (formData.selectedGame === "Valorant") {
        enforcedSize = 5;
      } else if (formData.selectedGame === "BGMI") {
        enforcedSize = 4;
      } else {
        enforcedSize = 4; // Default to BGMI size
      }
    }
  ...
  }, [event, teamRules, formData.selectedGame]);
  ...
  const getEventSpecificFields = () => {
    ...
    if (eventTitle.includes("battleclipse") || eventTitle.includes("esports")) {
      ...
      const getGameIdPlaceholder = () => {
        switch (formData.selectedGame) {
          case "BGMI":
            return "Enter BGMI Player IDs of all 4 team members (one per line)";
          case "Valorant":
            return "Enter Riot IDs of all 5 team members (one per line)";
          default:
            return "Enter game IDs of all team members (one per line)";
        }
      };
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-6">
            <div>
              ...
              >
                ...
                <option value="BGMI">BGMI (4 players required)</option>
                <option value="Valorant">Valorant (5 players required)</option>
              </select>
            </div>
          </div>
          ...
          {formData.selectedGame && (
            <div className="bg-accent-blue/10 border border-accent-blue/30 rounded-xl p-4">
              <p className="text-silver/80 text-sm">
                <strong>Team Requirements:</strong> This event requires exactly {getTeamSize()} team members. 
                {formData.selectedGame === "BGMI" && " For BGMI, provide Player IDs."}
                {formData.selectedGame === "Valorant" && " For Valorant, provide Riot IDs."}
              </p>
            </div>
          )}
          ...
          {formData.selectedGame && (
            <div className="grid grid-cols-1 gap-6">
              <div>
                <label className="block text-silver font-medium mb-2">
                  {getGameIdLabel()} <span className="text-red-400">*</span>
                </label>
                <textarea
                  required
    ...
    return (
      <div className="fixed inset-0 bg-charcoal/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-dark-blue/95 backdrop-blur-md border border-medium-blue/30 rounded-2xl p-8 text-center">
          ...
          <h3 className="text-xl font-bold text-silver mb-2">Loading Registration Form...</h3>
          <p className="text-silver/70">Please wait while we prepare the registration form</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-dark-blue/95 backdrop-blur-md border border-medium-blue/30 rounded-xl sm:rounded-2xl w-full max-w-6xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden shadow-2xl">
        ...
        <form onSubmit={(e) => { handleSubmit(e).catch(console.error); }} className="p-4 sm:p-6 overflow-y-auto max-h-[75vh]">
          <div className="space-y-8">
            ...
            {/* Team Information (hidden for individual-only events) */}
            {teamRules.maxSize !== 1 && (
              <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        >
                          ...
                          {(() => {
                            // For esports, show only the required team size based on selected game
                            if (eventTitle.includes("battleclipse") || eventTitle.includes("esports")) {
                              if (formData.selectedGame === "Valorant") {
                                return [5];
                              } else if (formData.selectedGame === "BGMI") {
                                ...
                                return [4]; // Default to BGMI
                              }
                            }
                            // For other events, use the normal range
                            return [1,2,3,4,5].filter(n => (teamRules.minSize ?? 1) <= n && n <= (teamRules.maxSize ?? 5));
                          })().map(n => (
          ...
          <div className="flex gap-4 mt-8 pt-6 border-t border-medium-blue/30">
            ...
            <button
              ...
              className={`flex-1 px-6 py-3 font-bold rounded-xl transition-all duration-300 flex items-center justify-center gap-2 ${
            ...
            >
              {isSubmitting ? (
                <>
                  <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-space-navy"></div>
